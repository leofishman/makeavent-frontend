<template>   
	<div v-if="ready" id="profile-company-new" :class="sponsorClass" :style="{ backgroundImage: `url('${api + bg}')` }">
...
<script>
	...
	export default {
		...
		methods: {
			...
			openPage (name) {
				this.$router.push(`/${this.$router.currentRoute.params.id}/${name}`).catch(() => {
