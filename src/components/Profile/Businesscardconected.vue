<template>
	<div class="businesscardconected">
		<p>
			<strong class="click" v-on:click="$root.tryBusinessCard(data)">{{data.name}}</strong>
			<span class="click has-text-grey" v-on:click="openCompany()">{{data.role}} {{content.at}} {{data.company}}</span>
		</p>
		<ul>
